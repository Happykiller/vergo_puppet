...
def test_train_siamese_model_nn():
    ...
    # Entraîner le modèle
    nn_model, losses = train_siamese_model_nn(training_data, word2idx, vocab_size, num_epochs=5)
...
def test_evaluate_similarity():
    ...
    # Entraîner le modèle
    nn_model, _ = train_siamese_model_nn(training_data, training_word2idx, vocab_size, num_epochs=5)
    ...
    word2idx = {word: idx for idx, word in enumerate(glossary)}
    similarity = evaluate_similarity(nn_model, seq1, seq2, word2idx)
    ...
    similarity_diff = evaluate_similarity(nn_model, seq1, seq3, word2idx)
...
def test_loss_decreases_with_continuous_labels():
    ...
    # Entraîner le modèle
    nn_model, losses = train_siamese_model_nn(training_data, word2idx, vocab_size, num_epochs=10)
...
def test_search_stability():
    ...
    for _ in range(5):
        ...
        for vector in dictionary:
            similarity = evaluate_similarity(nn_model, search_vector, vector, word2idx)
