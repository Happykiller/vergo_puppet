...
def train_lstm(dto: TrainLSTMUsecaseDto):
    ...
    joblib.dump(scaler, f'{dto.name}_scaler.pkl')
    joblib.dump(target_scaler, f'{dto.name}_target_scaler.pkl')
    joblib.dump(coco_encoder, f'{dto.name}_coco_encoder.pkl')

    # Model training
    nn_model = train_nn_lstm(X_train, y_train)
    
    # Update the model in storage
    bdd.update_model(dto.name, {"nn_model": nn_model})
