...
def test_training_result_initialization():
    """Test the initialization of a TrainingResult instance."""
...
def test_training_result_with_custom_timestamp():
    ...
    result = TrainingResult(model_name=model_name, metrics=metrics, timestamp=custom_timestamp)

    assert result.timestamp == custom_timestamp

def test_training_result_to_dict():
    """Test conversion of TrainingResult to dictionary."""
    model_name = "test_model"
    metrics = {"accuracy": 0.90, "loss": 0.05}
    timestamp = datetime(2024, 2, 6, 14, 30, tzinfo=timezone.utc)

    result = TrainingResult(model_name=model_name, metrics=metrics, timestamp=timestamp)
...
def test_training_result_from_dict():
    """Test reconstruction of TrainingResult from dictionary."""
    data = {
        "model_name": "test_model",
